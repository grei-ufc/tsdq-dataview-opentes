...
def carregar_dados(padrao_arquivo, base_value=None):
    """Carrega dados de um arquivo CSV e normaliza valores se base for fornecida"""
...
def render_visualizacao_3d(df_sub, eixo_x_sub, grupo_sub, df_carga, eixo_x_carga, grupo_carga):
    ...
    with st.expander("⚙️ Configurações do Gráfico 3D", expanded=True):
    ...
    with st.container():
        ...
        fig3d = go.Figure(data=[go.Surface(
            ...
            colorbar=dict(
                title=z_label,
                title_side='right'  # CORREÇÃO: title_side em vez de titleside
            ),
            contours={
    ...
    **Cálculo conforme PRODIST Módulo 8 - Item 5:**
    ...
    for idx, (tab, (nome, df)) in enumerate(zip(tabs, dados_disponiveis)):
        with tab:
            ...
            if fd_max > 3.0:
                st.warning("""
                **⚠️ ATENÇÃO: Desequilíbrio acima do limite permitido!**
    ...
    if len(dados_disponiveis) == 2:
        ...
        # Insights
        st.markdown("#### 🔍 Insights da Análise Comparativa")
